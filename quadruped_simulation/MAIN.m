...
prefisso = "Plot/";
modifica = "fricction 0.25 ";
cartella = modifica+"gait_"+string(gait)+"/"
prefisso = char(prefisso + cartella);
mkdir(prefisso)
...
nome_file = [prefisso 'pos.pdf'];
profmultiplot(t,X(:,1:3),Xd(:,1:3),'time[sec]','position [m]',legenda,nome_file);

% velocity
legenda={'$$v_x$$', '$$v_y$$', '$$v_z$$', '$$v_{xd}$$', '$$v_{yd}$$', '$$v_{zd}$$'};
nome_file = [prefisso 'vel.pdf'];
profmultiplot(t,X(:,4:6),Xd(:,4:6),'time[sec]','velocity [m/s]',legenda,nome_file);

% Angular velocity 
legenda={'$$\omega_x$$', '$$\omega_y$$', '$$\omega_z$$', '$$\omega_{xd}$$', '$$\omega_{yd}$$', '$$\omega_{zd}$$'};
nome_file = [prefisso 'angVel.pdf'];
profmultiplot(t,X(:,16:18),Xd(:,16:18),'time[sec]','angular velocity [rad/s]',legenda,nome_file);

% force control
real = [U2(:,3) U2(:,6) U2(:,9) U2(:,12)];
desired = [Ud(:,3) Ud(:,6) Ud(:,9) Ud(:,12)];
legenda = {'$$Fz_1$$', '$$Fz_2$$', '$$Fz_3$$', '$$Fz_4$$', '$$Fz_{1d}$$', '$$Fz_{2d}$$', '$$Fz_{3d}$$', '$$Fz_{4d}$$'};
nome_file = [prefisso 'Fz.pdf'];
profplot(t2,t,real,desired,'time[sec]','Fz [N]',legenda,nome_file);
